<script lang="ts">
    ...
    const handleMouseDown = (selectedNode: NodeId, details: { mousePosition: Vector2, nodePosition: Vector2 }) => {
        selectedNodeId = selectedNode;
        const mouseProjection = svgProjection(details.mousePosition.x, details.mousePosition.y);
            ...
            if(ports[0]) {
                ...
                if(targetId && targetName)
                    graphStore.addConnection(
                        { id: selectedConnection.id, name: selectedConnection.connectionName, },
    ...
    const handleMouseMove = (e: MouseEvent) => {
        if(selectedNodeId && selectedNodeOffset) {
            const {x, y} = svgProjection(e.clientX - selectedNodeOffset.x, e.clientY - selectedNodeOffset.y);
    ...
    const handleConnectionClick = (initialPosition: Vector2, id: NodeId, connectionType: InOutType, connectionName: string) => {
        selectedConnection = {
