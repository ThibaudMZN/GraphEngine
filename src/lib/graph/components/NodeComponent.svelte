<script lang="ts">
    ...
    type Props = {
        node: GraphNode;
        id: NodeId;
        handleMouseDown: (id: NodeId, details: { mousePosition: Vector2, nodePosition: Vector2 }) => void;
        handleConnectionClick: (initialPosition: Vector2, nodeId: NodeId, connectionType: InOutType, connectionName: string) => void;
    }
...
<rect
    ...
    onmousedown={(e) => handleMouseDown(id, {
...
{#each nodeDetails.inputs as input, index}
    ...
    {@const cy = count === 1 ? node.position.y : node.position.y - (height / 2) + (index * spacing)}
    ...
    <circle
        ...
        role="button"
        tabindex="0"
        onmousedown={() => handleConnectionClick({x: cx, y: cy}, id, input.type, input.name)}
    />
{/each}
{#each nodeDetails.outputs as output, index}
    ...
    <circle
        ...
        role="button"
        tabindex="0"
        onmousedown={() => handleConnectionClick({x: cx, y: cy}, id, output.type, output.name)}
    />
{/each}
