<script lang="ts">
    import {type Connection, GRAPH_NODE_HEIGHT, GRAPH_NODE_WIDTH, type GraphNode, graphStore} from "../GraphStore";
    ...
    const getInputOutputPoint = (node: GraphNode, socket: 'inputs' | 'outputs') => {
        ...
        const index = nodeDetails[socket]?.findIndex(i => i.name === connection[targetSocket].name) || 0;
        return count === 1
            ? node.position.y
    ...
    let path = $derived(`M ${points.x1} ${points.y1} C ${points.cx1} ${points.cy1}, ${points.cx2} ${points.cy2}, ${points.x2} ${points.y2}`);
</script>
